...
RSpec.describe Adyen::BalancePlatform, service: "Balance Platform service" do
  ...
  context "transaction" do
    ...
    it "retrieves information about a single transaction" do
      ...
      response_body = json_from_file("mocks/responses/BalancePlatform/get_transaction.json")

      url = client.service_url("BalancePlatform", "transactions/#{transaction_id}", "2")
      ...
      result = client.balance_platform.get_transaction(transaction_id)
      response_hash = result.response
  ...
  context "transfers" do
    xit "creates a transfer" do
      request_body = JSON.parse(json_from_file("mocks/requests/BalancePlatform/create_transfer_request.json"))

      result = client.balance_platform.create_transfer_request(request_body)
      response_hash = result.response
      puts response_hash
