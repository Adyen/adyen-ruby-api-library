module Adyen
  module Utils
    class HmacValidator
      ...
      def validate(notification_request_item, hmac_key)
        expected_sign = calculate_hmac(notification_request_item, hmac_key)
        merchant_sign = fetch(notification_request_item, 'additionalData.hmacSignature')

        expected_sign == merchant_sign
      end

      def calculate_hmac(notification_request_item, hmac_key)
      ...
      def data_to_sign(notification_request_item)
        VALIDATION_KEYS.map { |key| fetch(notification_request_item, key).to_s }
